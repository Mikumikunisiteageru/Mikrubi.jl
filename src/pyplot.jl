...
module Graphics end # deprecated

struct PlotModule end

Base.getproperty(::PlotModule, ::Symbol) = callsetplot(a...; kw...) = 
	error("Method undefined! Please import PyPlot.jl and `setplot(PyPlot)`.")
...
function showlayer(layer; f=identity, kwargs...)
	...
	handle = pcolor(xx, yy, f.(A); kwargs...)
	xlim(lim(xx))
	ylim(lim(yy))
	handle
end
...
"""
	showctpixels(ctpixels; salt=20, kwargs...)
...
"""
showctpixels(ctpixels; salt=20, kwargs...) = 
	showctpixels(ctpixels, ctpixels.indices; salt=salt, kwargs...)
